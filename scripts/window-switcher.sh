...
APP_CACHE="$CACHE_DIR/app-cache.txt"

mkdir -p "$CACHE_DIR"

# Load app cache: class<TAB>name<TAB>icon_path
declare -A CACHED_NAME=()
declare -A CACHED_ICON=()
if [[ -f "$APP_CACHE" ]]; then
  while IFS=$'\t' read -r c n i; do
    [[ -n "$c" ]] && CACHED_NAME["$c"]="$n" && CACHED_ICON["$c"]="$i"
  done < "$APP_CACHE"
fi
...
pretty_label_for_client() {
  ...
  # Check cache first
  if [[ -n "${CACHED_NAME[$cls]+x}" ]]; then
    name="${CACHED_NAME[$cls]}"
    icon_path="${CACHED_ICON[$cls]}"
  else
    ...
    # Output cache line to stdout for collection (will be captured by caller)
    printf 'CACHE:%s\t%s\t%s\n' "$cls" "$name" "$icon_path" >&2
  fi
...
# Collect cache entries during processing
cache_tmp="$(mktemp)"

while IFS=$'\t' read -r pid cls icls title ititle addr; do
  effective_cls="$cls"
  ...
  label="$(pretty_label_for_client "$effective_cls" "$title" 2>>"$cache_tmp")"
...
if [[ -s "$cache_tmp" ]]; then
  grep '^CACHE:' "$cache_tmp" | sed 's/^CACHE://' >> "$APP_CACHE" 2>/dev/null || true
fi
rm -f "$cache_tmp"
